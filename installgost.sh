...
function Install_ct() {
  ...
  if [[ ${addyn} == [Yy] ]]; then
    ...
    wget --no-check-certificate https://raw.githubusercontent.com/zhao123gg/gostandssr/main/gost-linux-amd64-2.11.1.gz
    gunzip gost-linux-"$bit"-"$ct_new_ver".gz
    mv gost-linux-"$bit"-"$ct_new_ver" gost
    mv gost /usr/bin/gost
    chmod -R 777 /usr/bin/gost
    wget --no-check-certificate https://raw.githubusercontent.com/zhao123gg/gostandssr/main/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system
    mkdir /etc/gost && wget --no-check-certificate https://raw.githubusercontent.com/zhao123gg/gostandssr/main/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  else
    rm -rf gost-linux-"$bit"-"$ct_new_ver".gz
...
update_sh() {
  ol_version=$(curl -L -s https://raw.githubusercontent.com/zhao123gg/gostandssr/main/installgost.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
  if [[ "$shell_version" != "$ol_version" ]]; then
    echo -e "存在新版本，是否更新；不更新直接回车 [Y/N]?"
    read -r update_confirm
    case $update_confirm in
    [yY][eE][sS] | [yY])
      wget -N --no-check-certificate https://raw.githubusercontent.com/zhao123gg/gostandssr/main/installgost.sh
      echo -e "更新完成"
      exit 0
      ;;
    *) ;;
